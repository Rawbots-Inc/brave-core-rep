...
//      }
  ...
  public func checkLogicCriteriaSatisfied(
    ...
    if !logicType.subCriteria.isEmpty {
      ...
      if mainCriteriaSatisfied {
        subCriteriaSatisfied = logicType.subCriteria.contains(
          where: checkSubCriteriaSatisfied(for:)
        )
      }
    }
  ...
  private func checkMainCriteriaSatisfied(
    for type: AppReviewMainCriteriaType,
    date: Date = Date()
  ) -> Bool {
    switch type {
    case .threshold:
  ...
  private func checkSubCriteriaSatisfied(for type: AppReviewSubCriteriaType) -> Bool {
    ...
    case .syncEnabledWithTabSync:
      return Preferences.Chromium.syncEnabled.value
        && Preferences.Chromium.syncOpenTabsEnabled.value
    }
  }
}
